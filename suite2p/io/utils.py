...
def get_nd2_list(ops):
    """
    if tiff_list is specified in ops, construct list of .nd2 files specified
    else construct a list of all nd2 files to process
        - if ops['look_one_level_down'], then obtain all nd2's in all folders + one level down
    """
    froot = ops["data_path"]
    fsall = []
    if "tiff_list" in ops:
        ...
        if nchannels > 1:
            ...
            if "keep_movie_raw" in ops and ops["keep_movie_raw"]:
                ops["raw_file_chan2"] = os.path.join(
                    ops["fast_disk"], "data_chan2_raw.bin"
                )
        if "dy" in ops and ops["dy"] != "":
            ops["dy"] = dy[j]
            ops["dx"] = dx[j]
